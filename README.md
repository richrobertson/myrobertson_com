...
- Source of truth for items is each published `blog/*.html` post with `BlogPosting` JSON-LD metadata (`headline`, `description`, `url`, `datePublished`). `datePublished` may be a date-only ISO string (`YYYY-MM-DD`) or a full ISO timestamp that includes `Z` or an explicit UTC offset; timestamps without a timezone are rejected. RSS generation uses the post URL as a stable tie-breaker when multiple posts share the same published instant.
